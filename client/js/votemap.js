...
VoteMap.prototype._createLastElectionResultButton = function() {
    L.control.custom({
        position: 'topright',
        content : '<button type="button" class="btn btn-1">'+
        ...
        events:{
            click(data) {
                ...
                if(clickMenu == "20대선거결과"){
					const lastElectionResultInfoTable = document.getElementsByClassName('last-party-info')[0];
					if(lastElectionResultInfoTable.style.display == "" || document.getElementsByClassName('last-party-info')[0].style.display == "none"){
						document.getElementsByClassName('last-party-info')[0].style.display = "table";
					}else{
						document.getElementsByClassName('last-party-info')[0].style.display = "none";
					}
                }
            }
        }
